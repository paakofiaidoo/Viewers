...
import GoogleCloudClient from './utils/GoogleCloudClient.js';
...
function createGCPDicomWebApi(gcpConfig, userAuthenticationService) {
  const {
    enableStudyLazyLoad,
  ...
  let qidoDicomWebClient, wadoDicomWebClient, gcpConfigCopy;
  const client = new GoogleCloudClient(gcpConfig);

  const implementation = {
    initialize: ({ params, query }) => {
      const { project, location, dataset, dicomStore } = params;

      const pathUrl = client.getUrlBaseDicomWeb(
        project,
        location,
        dataset,
        dicomStore
      ...
      gcpConfig.wadoRoot = pathUrl;
      gcpConfig.qidoRoot = pathUrl;
      gcpConfig.wadoUri = pathUrl;
      gcpConfig.wadoUriRoot = pathUrl;

      gcpConfigCopy = JSON.parse(JSON.stringify(gcpConfig));
      ...
      qidoDicomWebClient = new api.DICOMwebClient(qidoConfig);
      wadoDicomWebClient = new api.DICOMwebClient(wadoConfig);
    ...
    _retrieveSeriesMetadataAsync: async (
      ...
      function storeInstances(instances) {
        ...
        naturalizedInstances.forEach((instance, index) => {
          instance.wadoRoot = gcpConfig.wadoRoot;
          instance.wadoUri = gcpConfig.wadoUri;
    ...
    getImageIdsForInstance({ instance, frame }) {
      return getImageId({ instance, frame, config: gcpConfig });
    },
    getConfig() {
      return gcpConfigCopy;
    },
  };
